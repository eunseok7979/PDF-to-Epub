...
### 5.1 PP-DocLayout-M 카테고리 인식 문제
- **작동 확인**: text, number, header
- **미작동/미확인**: figure, table, footer, 기타 (23개 중 대부분 미검증)
- **원인 미확인**: 모델 자체의 한계인지, 파싱 로직 문제인지 구분 필요
- **다음 단계**: 다양한 레이아웃의 테스트 PDF로 카테고리별 체계적 테스트
